...
const { user, reset_password, incorrectUser, login_url, toppage_url } = require("../../utils/credentials");
...
test("validate login [ Error message ]", async ({ page }) => {
  ...
  await page.getByPlaceholder("テナントIDを入力してください").fill(incorrectUser.tenant_id);
  await page.getByPlaceholder("登録したメールアドレスを入力してください").fill(incorrectUser.email);
...
test.describe("validate change password pop-up [ Error message ]", () => {
  test("If user focus out of fields", async ({ page }) => {
    ...
    await expect(page.getByText("以下のパスワード条件を満たす必要があります。")).toBeVisible();
    await expect(page.getByText("パスワードを入力してください。", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "キャンセル" }).click();
  });

  test("If user inputs less than 14 and more than 99 characters", async ({ page }) => {
    await login_success(page);
    await page.getByText("防災 太郎").click();
    await page.getByRole("banner").getByText("パスワード変更").click();
    ...
    //check valid icon
    await expect(page.locator('li').filter({ hasText: '文字以上、99文字以下' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
    await expect(page.locator('li').filter({ hasText: '記号とスペースは使用しない' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-valid');
    await expect(page.locator('li').filter({ hasText: '英大文字、英小文字、半角数字を含む' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-valid');
  ...
  test("If user inputs symbols", async ({ page }) => {
    ...
    await expect(page.locator('li').filter({ hasText: '文字以上、99文字以下' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
    await expect(page.locator('li').filter({ hasText: '記号とスペースは使用しない' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
    await expect(page.locator('li').filter({ hasText: '英大文字、英小文字、半角数字を含む' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-valid');
  ...
  test("If user inputs only lowercase letter", async ({ page }) => {
    ...
    await expect(page.locator('li').filter({ hasText: '文字以上、99文字以下' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
    await expect(page.locator('li').filter({ hasText: '記号とスペースは使用しない' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
    await expect(page.locator('li').filter({ hasText: '英大文字、英小文字、半角数字を含む' }).locator('i')).toHaveClass('sb-icon sb-icon-xs sb-icon-check-invalid');
